...
public class ProjectStore {
    ...
    public void dispose(IJavaProject project) {
        String path = project.getProject().getFullPath().toString();
        JavaProject jproject = projectStore.get(path);
        if (jproject != null) {
            jproject.dispose();
    ...
    void removeFile(IFile file) {
        JavaProject jproject = projectStore.get(file.getProject().getFullPath().toString());
        if (jproject == null) {
            return;
        }
    ...
    public void build(IFile file) {
        ...
        if (jproject == null || jproject.getFiles().size() == 0) {
            ...
            IJavaProject project = JavaCore.create(file.getProject());
            jproject = projectStore.get(project.getPath().toString());
            if (project != null) {
    ...
    public void build(IJavaProject project) {
        JavaProject jproject = projectStore.get(project.getPath().toString());
        if (jproject == null || jproject.getFiles().size() == 0) {
            buildWhole(project);
            return;
        }
    ...
    private void buildWhole(IJavaProject project) {
        ...
        if (cu != null) {
            ...
            if (code != null) {
                ...
                if (getParseErrors(cu).size() == 0) {
                    printLog("Complete parse: " + icu.getPath().toString());
                } else {
                    printError("Incomplete parse: " + icu.getPath().toString());
                }
